...
class Application(Tk):
	# 初始化棋盘,默认九路棋盘
	def __init__(self,my_mode_num=5):
		Tk.__init__(self)
		...
		# 相对九路棋盘的矫正比例
		self.p=1 if self.mode_num==5 else (2/3 if self.mode_num==6 else 4/9)
		...
		self.passmeButton=Button(self,text='让一次棋',command=self.passme)
		self.passmeButton.place(x=480*self.size,y=225*self.size)	
		self.regretButton=Button(self,text='悔一次棋',command=self.regret)
		...
		self.newGameButton1=Button(self,text=('六' if self.mode_num==5 else '五')+'路棋',command=self.newGame1)
		self.newGameButton1.place(x=480*self.size,y=300*self.size)
		self.newGameButton2=Button(self,text=('六' if self.mode_num==7 else '七')+'路棋',command=self.newGame2)
		...
		for m in [-1,0,1]:
			for n in [-1,0,1]:
				...
				self.canvas_bottom.move(self.oringinal,m*self.dd*(2 if self.mode_num==5 else (3 if self.mode_num==6 else 6)),
				n*self.dd*(2 if self.mode_num==5 else (3 if self.mode_num==6 else 6)))
		# 画中间的线条
	...
	def newGame1(self):
		...
		mode_num=(6 if self.mode_num==5 else 5)
		newApp=True
		self.quit()
	def newGame2(self):
		global mode_num,newApp
		mode_num=(6 if self.mode_num==7 else 7)
		newApp=True
		self.quit()
...
def main():
	if __name__=='__main__':
		...
		while True:
			newApp=False
			app=Application(mode_num)#用类进行工作
			app.title('围棋')
			app.mainloop()
			if newApp:
				app.destroy()
